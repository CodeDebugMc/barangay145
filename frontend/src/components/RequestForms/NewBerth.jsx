...
export default function NewBerth() {
  // API base - adjust to your backend
  ...
  // form fields matching bhert_normal
  const emptyForm = {
    resident_id: "",
    name: "",
    address: "",
  ...
  async function loadRecords() {
    try {
      const res = await fetch(`${apiBase}/bhert-normal`);
      const data = await res.json();
      const mapped = Array.isArray(data)
        ? data.map((r) => ({
            ...
            name: r.name || "",
            address: r.address || "",
            request_hospital: r.request_hospital || "",
            date_issued: r.date_issued ? r.date_issued.split("T")[0] : "",
            transaction_number:
              r.transaction_number || generateTransactionNumber("BH"),
  ...
  function generateTransactionNumber(prefix = "BH") {
    const date = new Date();
  ...
  // ----- QR generation (document type corrected) -----
  useEffect(() => {
...
Document Type: Birth Certification
Barangay 145, Caloocan City`;
  ...
  // ----- payload mapping (matches bhert_normal columns) -----
  function toServerPayload(d) {
    return {
      resident_id: d.resident_id || null,
      name: d.name || "",
      address: d.address || "",
      request_hospital: d.request_hospital || null,
  ...
  async function handleCreate() {
    try {
      const tx = formData.transaction_number || generateTransactionNumber("BH");
      ...
      // server should ideally return created row including id; fallback to optimistic id
      const newRec = {
        ...payload,
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/bhert-normal/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  async function handleDelete(id) {
    ...
    try {
      const res = await fetch(`${apiBase}/bhert-normal/${id}`, {
        method: "DELETE",
      });
  ...
  // store certificate for QR verification
  ...
  // ----- PDF generation & print (use id 'bhert-certificate') -----
  async function generatePDF() {
    ...
    try {
      const el = document.getElementById("bhert-certificate");
      ...
      const fileName = `BirthCert_${display.id || "draft"}_${(
        display.name || ""
      ).replace(/\s+/g, "_")}.pdf`;
      pdf.save(fileName);
    } catch (err) {
              ...
              >
                ...
                >
                  ...
                  <p style={{ textIndent: "50px" }}>
                    ...
                    is NOT INCLUDED in the list of household in this barangay
                    who is being monitored with{" "}
                    <span
                      style={{
                        color: "red",
                        fontWeight: "bold",
                    ...
                    is NOT INCLUDED in the list of persons who is being
                    monitored in this barangay to the PUI or CONFIRMED with
                    COVID-19. No symptoms of Covid-19 virus.
                  </p>
                  <p style={{ textIndent: "50px" }}>
                    ...
                    , for whatever legal purposes it may serve. Done in the
                    office of the Punong Barangay 145, Zone 13, District 1, City
                    of Caloocan this{" "}
                    <span
                      style={{
                        color: "red",
                ...
                >
                  ...
                  <div style={{ textAlign: "center", width: 300 }}>
                    ...
                    <div style={{ fontWeight: 700 }}>Punong Barangay</div>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <CardHeader
                    title={
                      <Typography sx={{ fontWeight: 600 }}>
                        {editingId ? "Edit Birth Record" : "New Birth Record"}
                      </Typography>
                    }
                    subheader={
                      selectedRecord &&
                  ...
                  <CardContent>
                    <Stack spacing={2}>
                      ...
                      <TextField
                        ...
                        size="small"
                        fullWidth
                        value={formData.request_hospital}
                        onChange={(e) =>
                          setFormData((p) => ({
                            ...p,
                            request_hospital: e.target.value,
                          }))
                        }
                      />
                      <TextField
                  ...
                  <CardContent>
                    <Box sx={{ display: "flex", gap: 1, mb: 2 }}>
                      <TextField
                        fullWidth
                        size="small"
                        placeholder="Enter transaction no (e.g. BH-231001-123)"
