...
app.delete("/solo-parent/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await pool.query(
...
// BHERT NORMAL
//===============
app.get("/bhert-normal", async (req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT id,
        resident_id,
        ...
        is_active,
        date_created,
        date_updated FROM bhert_normal WHERE is_active = TRUE ORDER BY id DESC`);
    res.json(rows);
  } catch (err) {
    console.error("Database Query Error:", err.message);
    res.status(500).json({ error: "Internal Server Error" });
...
app.get("/bhert-normal/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query(
      `  SELECT id,
...
app.get("/bhert-normal/:transactionNumber", async (req, res) => {
  try {
    const { transactionNumber } = req.params;
    const [rows] = await pool.query(
      `  SELECT id,
...
app.post("/bhert-normal", async (req, res) => {
  try {
    const {
      resident_id,
      name,
    ...
    const sql = `INSERT INTO bhert_normal (resident_id,
      name,
      address,
      request_hospital,
      date_issued,
...
app.put("/bhert-normal/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const {
      resident_id,
    ...
    const sql = `UPDATE bhert_normal SET  resident_id = ?,
      name = ?,
      address = ?,
      request_hospital = ?,
      date_issued = ?,
...
app.delete("/bhert-normal/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const [result] = await pool.query(
      `UPDATE bhert_normal SET is_active = FALSE, date_updated = NOW() WHERE id = ?`,
      [id]
    );
